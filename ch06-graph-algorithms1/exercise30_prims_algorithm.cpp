// g++ exercise30_prims_algorithm.cpp -o exercise30_prims_algorithm.exe
// exercise30_prims_algorithm.exe
...
MST Krusal: preferred for sparse graphs with a small number of edges
MST Prim: preferred for dense graphs with a large number of edges
*/
...
struct Label
{
	...
	Label(size_t _id, T _distance) : vertex_ID(_id), distance_from_frontier(_distance)
	{}
...
auto prim_MST(const Graph<T>& G, size_t src)
{
	...
	while (!heap.empty())
	{
		...
		{
			std::cout << "Settling vertex ID " << current_vertex.vertex_ID << std::endl;
